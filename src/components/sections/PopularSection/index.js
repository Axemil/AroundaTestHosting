...
let PopularSection = ({ postsInter, fetchPostsInteresting, currentPostId }) => {
  React.useEffect(() => {
  ...
  return !morePosts ? (
    ...
    <div className={style.MoreInteresting}>
      <div className={style.wrapTop}>
        <h1 className={style.title}>Popular articles</h1>
      ...
      <div className={style.advices}>
        ...
        <Carousel cols={4} rows={1} loop gap={40.5}>
        ...
        <div className={style.mobilePopular}>
        {morePosts.map((post, i) => {
          if(i < 6)
          return(
            <a key={i} href={`/blog/${post.slug}/`}>
...
PopularSection = connect(mapStateToProps, { fetchPostsInteresting })(PopularSection)
